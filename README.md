...
Requires only Docker — no local PostgreSQL or Python.
...
curl -s "localhost:8000/galaxies/?limit=5"
```

`DATABASE_URL` configures both the API and the loader. Teardown:
`docker rm -f api db && docker network rm galaxynet` (data is not persisted).
